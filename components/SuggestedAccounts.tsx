...
const SuggestedAccounts: NextPage<IProps> = ({ fetchAllUsers, allUsers }) => {
  ...
  return (
    <div className='xl:border-b-2 border-gray-200 dark:border-gray-700 pb-4'>
      ...
      <div>
        {users?.slice(0, 6).map((user: IUser) => (
          <Link href={`/profile/${user._id}`} key={user._id}>
            <div className='text-primary dark:text-white flex gap-3 hover:bg-primary dark:hover:bg-gray-700 p-2 cursor-pointer font-semibold rounded'>
              ...
              <div className='hidden xl:block'>
                ...
                <p className='capitalize text-gray-400 dark:text-gray-500 text-xs'>
                  {user.userName}
                </p>
              </div>
            </div>
